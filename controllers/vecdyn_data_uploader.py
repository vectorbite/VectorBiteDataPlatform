...
def vecdyn_importer():
    """At the moment the importer imports fields based on their position om the CSV, rather than using field names.
    Eventually this should import data based on field names"""
        ...
        try:
            ...
            for row in readCSV:
                ...
                if (check1 == None) | (check1 == check2):
                    ...
                    pass
                # A similar checker should also be implemented for the time series data
                # to avoid duplicate time series entries, although this could take up a lot of memory
            ...
            # This is quite bad practice!
            # It would be better to except a particular exception and handle it appropriately
            # Otherwise things fail silently
            response.flash = 'Errors, no data submitted. ' \
                             'Please ensure the data set meets all the validation requirements, ' \
                             'hit cancel to go back to data collections page'
